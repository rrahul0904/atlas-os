...
export function renderIntegrationsPage(input:{
  ...
  const webhookCard=`
    <section class="integration-card">
      <div class="integration-head">
        ...
        <span class="status ${status==="connected"?"good":status==="degraded"?"warn":status==="error"?"bad":"muted"}">${esc(statusLabel(status))}</span>
      </div>
  ...
  const googleCard=`
    <section class="integration-card">
      <div class="integration-head">
        <div><small class="eyebrow">COMING NEXT</small><h2>Google Workspace</h2></div>
        <span class="status muted">not configured</span>
      </div>
      <dl class="integration-meta">
        <div><dt>Account</dt><dd>Not connected</dd></div>
        <div><dt>Capabilities</dt><dd>Gmail · Calendar</dd></div>
      </dl>
      <p class="muted-copy">OAuth is intentionally not activated until the generic integration execution path is certified.</p>
    </section>`;
