...
const required=[
  ...
  "atlas_schema_migrations","atlas_evidence","atlas_action_items","atlas_agents","atlas_workflow_definitions","atlas_workflow_runs","atlas_workflow_step_runs"
];
...
try{
  ...
  if(Number(migrations[0]?.count)!==7) throw new Error(`Expected 7 migrations, found ${migrations[0]?.count ?? 0}`);
  console.log(`Database contract OK: ${required.length} tables, 7 migrations.`);
}finally{
